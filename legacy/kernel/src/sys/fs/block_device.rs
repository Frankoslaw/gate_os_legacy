...
pub static BLOCK_DEVICE: Spinlock<Option<BlockDevice>> = Spinlock::new(None);
...
const ATA_CACHE_SIZE: usize = 128;

#[derive(Clone)]
pub struct AtaBlockDevice {
    cache: [Option<(u32, Vec<u8>)>; ATA_CACHE_SIZE],
    dev: sys::drivers::ata::Drive,
}

impl AtaBlockDevice {
    pub fn new(bus: u8, dsk: u8) -> Option<Self> {
        sys::drivers::ata::Drive::open(bus, dsk).map(|dev| {
...
impl BlockDeviceIO for AtaBlockDevice {
    fn read(&mut self, block_addr: u32, buf: &mut [u8]) -> Result<(), ()> {
        ...
        sys::drivers::ata::read(self.dev.bus, self.dev.dsk, block_addr, buf)?;
        self.set_cached_block(block_addr, buf);
        Ok(())
    }

    fn write(&mut self, block_addr: u32, buf: &[u8]) -> Result<(), ()> {
        sys::drivers::ata::write(self.dev.bus, self.dev.dsk, block_addr, buf)?;
