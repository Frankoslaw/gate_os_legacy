...
impl Bus {
    ...
    fn poll(&mut self, bit: Status, val: bool) -> Result<(), ()> {
        ...
        while self.status().get_bit(bit as usize) != val {
            if sys::clock::uptime() - start > 1.0 {
                log::info!("ATA hanged while polling {:?} bit in status register", bit);
                self.debug();
                return Err(());
            }
            spin_loop();
    ...
    fn debug(&mut self) {
        unsafe {
            log::info!("ATA status register: 0b{:08b} <BSY|DRDY|#|#|DRQ|#|#|ERR>", self.alternate_status_register.read());
            log::info!("ATA error register:  0b{:08b} <#|#|#|#|#|ABRT|#|#>", self.error_register.read());
        }
    }
}

lazy_static! {
    pub static ref BUSES: Spinlock<Vec<Bus>> = Spinlock::new(Vec::new());
}
