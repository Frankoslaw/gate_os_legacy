...
pub fn init(info: &Framebuffer) {
    ...
    display.buffer = unsafe { core::slice::from_raw_parts_mut(info.addr(), display.width * display.height) };
    ...
    display.write_rgba(include_bytes!(concat!(env!("OUT_DIR"), "/ferris-unsafe.rgba")));
